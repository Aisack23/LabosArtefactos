...
void loop() {
  // Revisa si WiFi sigue conectado
  if (WiFi.status() != WL_CONNECTED) {
    conectarWiFi();
  }
  ...
  // Publicar estado de alarma solo si cambia
  static bool ultimoObstaculo = false;
  if (obstaculo != ultimoObstaculo) {
    if (obstaculo) {
  ...
  // Cada 10 segundos → respeta límite de Adafruit IO
  delay(10000);
}
